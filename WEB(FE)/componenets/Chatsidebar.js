...
export async function getServerSideProps(userId) {
  ...
  const res = await fetch(endpoint + userId, options);
  const data = await res.json();

  return { props: { data } };
}

function Menuelement({ userId, snapshot }) {
      ...
      <div>
        <div className={styles.firstrow}>
          ...
          <p className={styles.date}>
            {new Date(thechat.rectime.seconds * 1000).toLocaleString([], {
              hourCycle: "h23",
              year: "numeric",
              month: "2-digit",
              day: "2-digit",
        ...
        <p className={styles.content}>{thechat.recentmsg}</p>
        <p
          style={{ margin: "0px" }}
          className={(() => {
            console.log(typeof thechat.severity);
            if (thechat.severity == "1") return styles.severityLevel1;
            if (thechat.severity == "2") return styles.severityLevel2;
            if (thechat.severity == "3") return styles.severityLevel3;
            if (thechat.severity == "4") return styles.severityLevel4;
            if (thechat.severity == "5") return styles.severityLevel5;
            return "";
          })()}
        >
          중요도: {thechat.severity}
        </p>
      </div>
    </div>
  ));
...
const Sidebar = ({ userId, snapshot, children }) => {
  ...
  let submitnewchat = async (event) => {
    try {
      let gotuserdata = await getServerSideProps(userId);
    ...
    <>
      ...
      >
        <Form onFinish={submitnewchat}>
          ...
          <Form.Item name="채팅 이름" rules={[{ required: true }]}>
            <Input
              style={{ width: "440px", marginBottom: "10px" }}
              onChange={(event) => {
          ...
          <Addperson
            ...
            fetchedInvitedList={fetchedInvitedList}
            setFetchedInvitedList={setFetchedInvitedList}
          ></Addperson>
      ...
      <div className={styles.flexcontainer}>
        <div className={styles.sidebar}>
          <Menuelement snapshot={snapshot} userId={userId} />
        </div>
        <div className={styles.chatroom}>{children}</div>
      </div>
    </>
  );
};
